...
def interpolate_xs(energy: np.ndarray, xs_energy: np.ndarray, xs: np.ndarray):
    """Interpolate cross section data onto supplied energy grid using logarithmic interpolation.

    Parameters
    ----------
    energy : np.ndarray
        Energy points at which to interpolate the cross section.
    xs_energy : np.ndarray
    ...
    -------
    np.ndarray
        Interpolated cross section values at the requested energy points.
